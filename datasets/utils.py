...
class DatasetBase:
    ...
    def download_data(self, url, dst, from_gdrive=True):
        ...
        if from_gdrive:
            raise NotImplementedError("Google Drive download not supported in Jittor.")
        else:
...
class DatasetWrapper(Dataset):
    def __init__(self, data_source, batch_size,  tfm=None, is_train=False, shuffle=False, input_size=224,
                 return_img0=False, k_tfm=1):
        super().__init__()
        self.data_source = data_source
        self.transform = tfm # accept list (tuple) as input
        ...
        if self.k_tfm > 1 and tfm is None:
            raise ValueError(
        ...
        mode = Image.BICUBIC
        to_tensor = []
        to_tensor += [jt_transform.Resize(input_size, mode)]
        to_tensor += [jt_transform.ToTensor()]
